<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Dashboard >')}} 
            <x-nav-link :href="route('rules-title.list')">
                {{ __('Rules Title List') }}
            </x-nav-link>
            @if(!empty($row->id))
            {{ __('> Edit Rules Status') }}
            @else
            {{ __('> Add Rules Status') }}
            @endif
        </h2> 
        ...
        <div class="col-md-8">
            <div class="container w-full md:w-5/5 xl:w-5/5  mx-auto px-2">
                <div id='recipients' class="p-4 mt-3 lg:mt-0 rounded shadow bg-white">
                    ...
                    @if(!empty($row->id))
                    <form method="POST" action="{{ url('rules-title/update/'.$row->id) }}"> 
                        @else
                        <form method="POST" action="{{ route('rules-title.add') }}">
                            @endif
                            @csrf
                            <div class="row">
                                <div class="col">
                                    <div class="form-group row">
                                        <label class="col-sm-3 col-form-label">Rules Title:</label>
                                        <div class="col-sm-9">
                                            <x-input class="block mt-1 w-full" type="text" name="title" :value="old('title', isset($row->title)? $row->title : '' )" required autofocus placeholder="Enter Rules Title"/>
                                    </div>
                                </div>
                            </div>
                        </div>
