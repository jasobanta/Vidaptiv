                ...
                <table id="example">
                    <thead>
                        <tr>
                            ...
                            <th>Carrier</th>
                            <th>Status</th>
                            <th class="notexport">Sent EDI</th>
                            <th class="notexport">Rec. EDI</th>
                            <th class="notexport">Diff.</th>
                        </tr>
                        <tr style="width: 100%;padding: 3px;box-sizing: border-box;">
                            <td><input type="hidden" name="search_all" value="search_all"><input type="text" class="form-control" style="" name="dtm" id="dtm"></td>
                            <td><input type="text" class="form-control" style="" name="received_date" id="received_date"></td>
                            <td></td>
                            <td><input type="text" class="form-control" style="" name="owner_name"></td>
                            <td>
                                <select name="country_code" id="" class="form-control"  style="width:90px;">
                                    <option value="">Select</option>
                                    @foreach($all_countries as $single_country)
                                        <option value="{{$single_country['country_code']}}">{{$single_country['country_code']}}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td><input type="text" class="form-control" style="" name="booking_no"></td>
                            <td><input type="text" class="form-control" style="" name="ff_no"></td>
                            <td><input type="text" class="form-control" style="width:75px;" name="bn_no"></td>
                            <td>
                                <select name="carrier" id="" class="form-control"  style="width:90px;">
                                    <option value="">Select</option>
                            ...
                            <td>
                                <select name="status" id="" class="form-control"  style="width:90px;">
                                    <option value="">Select</option>
                                    @foreach($edi_status as $edi_status_key=>$single_edi_status)
                                        <option value="{{$edi_status_key}}">{{$single_edi_status}}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td></td>
...
<script>
    $(document).ready(function () {
        ...
        let table = $('#example').DataTable({
            ...
            'serverMethod': 'post',
            'ajax': {
                'url': <?php echo "'".route('dashboard.datatable.ajax')."'"; ?> ,
                'headers': {
        ...
        //$("#example_wrapper").prepend('<div id="example_filter" class="text-right" style="float:right;"><label style="margin-left: 30px;">Grid Filter: <input type="search" id="searchInput" class="" placeholder="" aria-controls="example"></label></div>');
        ...
        $('#dtm').on('cancel.daterangepicker', function(ev, picker) {
