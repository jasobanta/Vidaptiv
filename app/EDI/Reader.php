...
class Reader
{
   ...
   public function validateDate($date, $format = 'Y-m-d H:i:s'){
    ...
    public function getUNBEdiDate(){
       try{
            $datetime = collect($this->dataExtractor(['UNB']))->flatten();
 ...
 public function dataExtractor2($filters, $data=[]){
       	$dataExtract = [];
	if(empty($data)){
	$data = $this->getParsedFile();
	}
	foreach($data as $segNo=>$ediSegment){
             // let's see how deep is the filter ask to get the data
		$filterCount = \count($filters);
		switch($filterCount):
		
		case 1:
		$seg = array_shift($ediSegment);
		If($seg === $filters[$filterCount-1])
		$dataExtract[$segNo]=collect($ediSegment)->flatten()->toArray();
		break;
		
		case 2:
                $seg = array_shift($ediSegment);
		$subSeg = array_shift($ediSegment);
		if(is_array($subSeg)){
		$subSeg_arr = array_shift($subSeg);
		if($seg === $filters[$filterCount-2] && $subSeg_arr === $filters[$filterCount-1]){
		$dataExtract[$segNo]= collect($subSeg)->flatten()->toArray();
		}
		}else{
		if($seg === $filters[$filterCount-2] && $subSeg === $filters[$filterCount-1])
		$dataExtract[$segNo]= collect($ediSegment)->flatten()->toArray();
		}
		break;
		...
		$seg = array_shift($ediSegment); 
                $subSeg = array_shift($ediSegment); 
                $identifier = array_shift($ediSegment);
		if($seg === $filters[$filterCount-3] && $subSeg === $filters[$filterCount-2] && $identifier = $filters[$filterCount-1] )
		 $dataExtract[$segNo]= collect($ediSegment)->flatten()->toArray();
		break;
		default :
		break;
...
public function dataExtractor($filters, $data=[]){
	$dataExtract = [];
	if(empty($data)){
	$data = $this->getParsedFile();
	}
	foreach($data as $segNo=>$ediSegment){
		...
		$seg = array_shift($ediSegment);
		If($seg === $filters[$filterCount-1])
		$dataExtract[$segNo]=array_flip(array_change_key_case(array_flip(collect($ediSegment)->flatten()->toArray()),CASE_UPPER));
		break;
		...
		$subSeg_arr = array_shift($subSeg);
		if($seg === $filters[$filterCount-2] && $subSeg_arr === $filters[$filterCount-1]){
		$dataExtract[$segNo]=array_flip(array_change_key_case(array_flip(collect($subSeg)->flatten()->toArray()),CASE_UPPER));
		}
		}else{
		if($seg === $filters[$filterCount-2] && $subSeg === $filters[$filterCount-1])
		$dataExtract[$segNo]=array_flip(array_change_key_case(array_flip(collect($ediSegment)->flatten()->toArray()),CASE_UPPER));
